...
@bot.command(pass_context = True)
async def rank (ctx, mem: discord.Member, *, ro: discord.Role):
 if "Admin" in [ro.name for ro in ctx.message.author.roles]:
     author = ctx.message.author
     if mem is None:
        mem = author
     if ro in mem.roles:
        await bot.say("The user already has that role :no_entry:")
        return
     try:
        await bot.add_roles(mem, ro)
        await bot.say("**{}** has been added to **{}**.".format(ro, mem))
     except discord.errors.Forbidden:
        await bot.say("I'm not able to add the role to the user :no_entry:")
 else:
            await bot.say ("You do not have permission to use this command! You need to have the 'admin' rank!")
